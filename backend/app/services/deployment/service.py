"""
배포 작업 통합 서비스 모듈 (deployment 도메인)

기존 app.services.deployment_service 모듈을
...
class DeploymentService:
                    ...
                    if not deploy_request.get("template_id"):
                        ...
                        if cloudinit_user_data:
                            ...
                            task_manager.append_log(
                                task_id, "Cloud-init user-data 생성 완료 (SSH 키 자동 주입)"
                            )
                        else:
                            task_manager.append_log(
                                task_id,
                                "경고: SSH 공개키를 찾을 수 없어 Cloud-init을 사용하지 않습니다",
                            )
    ...
    def _generate_cloudinit_user_data(self) -> Optional[str]:
        """
        Cloud-init user-data YAML 생성
        - SSH 공개키를 자동으로 읽어서 user-data에 포함
        """
        ...
        yaml_str = yaml.dump(user_data, default_flow_style=False)
