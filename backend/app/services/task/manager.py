"""
작업 상태 관리 모듈 (task 도메인)

기존 app.services.task_manager 모듈을
...
class TaskManager:
    ...
    def __init__(self):
        ...
        self._storage_path = Path(__file__).resolve().parents[3] / "data" / "task_history.json"
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            loaded_logs = payload.get("logs", {})
            if isinstance(loaded_tasks, dict):
                self._tasks = loaded_tasks
            if isinstance(loaded_logs, dict):
                normalized_logs: Dict[str, List[str]] = {}
                for task_id, lines in loaded_logs.items():
                    if isinstance(lines, list):
                        normalized_logs[str(task_id)] = [str(line) for line in lines]
                self._logs = normalized_logs

            for task_id in self._tasks.keys():
                self._logs.setdefault(task_id, [])
                self._ensure_task_shape_unlocked(task_id)
        except Exception:
            # 로드 실패 시 메모리 상태로 계속 동작
            self._tasks = {}
            self._logs = {}
        ...
        with self._lock:
            if task_id in self._tasks:
                ...
                self._persist_state_unlocked(
                    force=normalized_status in self._DONE_STATUSES
                )
